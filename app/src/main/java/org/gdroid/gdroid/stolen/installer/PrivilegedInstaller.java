...
public class PrivilegedInstaller //extends Installer
{
    ...
    private final ApplicationBean apk;

    public PrivilegedInstaller(Context context, @NonNull ApplicationBean apk) {
        //super(context, apk);
...
//    }
        ...
        ServiceConnection mServiceConnection = new ServiceConnection() {
            public void onServiceConnected(ComponentName name, IBinder service) {
                ...
                try {
                    ...
                    privService.deletePackage(apk.id, 0, callback);
                } catch (RemoteException e) {
                    Log.e(TAG, "RemoteException", e);
                    sendBroadcastUninstall(ACTION_UNINSTALL_INTERRUPTED,
                            "connecting to privileged service failed");
    ...
    static void sendBroadcastInstall(Context context,
                                     Uri downloadUri, String action, ApplicationBean apk,
                                     PendingIntent pendingIntent, String errorMessage) {
        Intent intent = new Intent(action);
        intent.setData(downloadUri);
        intent.putExtra(EXTRA_USER_INTERACTION_PI, pendingIntent);
//        intent.putExtra(Installer.EXTRA_APK, apk); // TODO bean
    ...
    static void sendBroadcastUninstall(Context context, ApplicationBean apk, String action) {
        sendBroadcastUninstall(context, apk, action, null, null);
    }

    private static void sendBroadcastUninstall(Context context, ApplicationBean apk, String action,
                                               PendingIntent pendingIntent, String errorMessage) {
        Uri uri = Uri.fromParts("package", apk.id, null);
